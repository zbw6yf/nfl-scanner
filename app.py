...
def fetch_action_network_lines() -> Dict[str, Dict]:
    """
    Open vs current lines from Action Network scoreboard.
    Open = earliest inserted type=game book line; Current = latest inserted.
    Keyed by AWAY_HOME (our abbrs).
    """
    out: Dict[str, Dict] = {}
    try:
        r = requests.get(
        ...
        abbr_fix = {
            "JAC": "JAX", "LAR": "LA", "WSH": "WAS", "WAS": "WAS",
            "GNB": "GB", "KAN": "KC", "NOR": "NO", "NWE": "NE",
            "SFO": "SF", "TAM": "TB", "OAK": "LV", "LVR": "LV", "SD": "LAC",
        }
        for g in games:
            teams = g.get("teams") or []
            if len(teams) < 2:
                continue
            # Action Network lists away then home in teams array order matching away_team_id
            ...
            away = str(away_t.get("abbr") or "").upper()
            home = str(home_t.get("abbr") or "").upper()
            away = abbr_fix.get(away, away)
            home = abbr_fix.get(home, home)
            away = _normalize_team_abbr(away) or away
            home = _normalize_team_abbr(home) or home
            odds = [
                o for o in (g.get("odds") or [])
                if o.get("type") == "game"
                and o.get("spread_home") is not None
            ]
            if not odds:
                continue
            open_o = min(odds, key=lambda o: str(o.get("inserted") or ""))
            cur_o = max(odds, key=lambda o: str(o.get("inserted") or ""))
            try:
                open_s = float(open_o.get("spread_home"))
            except Exception:
                open_s = None
            try:
                open_t = float(open_o.get("total")) if open_o.get("total") is not None else None
            except Exception:
                open_t = None
            try:
                cur_s = float(cur_o.get("spread_home"))
            except Exception:
                cur_s = None
            try:
                cur_t = float(cur_o.get("total")) if cur_o.get("total") is not None else None
            except Exception:
                cur_t = None
            key = f"{away}_{home}"
            out[key] = {
...
def resolve_open_lines(
    ...
    """
    Open = earliest available market line (historical Odds API preferred, else first stored).
    Current = latest live line only (never copied onto open once open is set).
    """
    line_key = f"{away}_{home}_{str(gameday)[:10]}"
    opens = _load_line_opens()
    entry = dict(opens.get(line_key) or {})
    source = entry.get("source") or "none"

    # Parse live current
    try:
        cur_s = float(cur_spread) if cur_spread is not None and str(cur_spread) not in ("", "None", "nan", "—") else None
    except Exception:
        cur_s = None
    try:
        cur_t = float(cur_total) if cur_total is not None and str(cur_total) not in ("", "None", "nan", "—") else None
    except Exception:
        cur_t = None

    # Existing stored open (immutable once real numbers exist)
    try:
        open_s = float(entry["open_spread"]) if entry.get("open_spread") is not None and str(entry.get("open_spread")) not in ("", "nan", "None") else None
    except Exception:
        open_s = None
    ...
    # Historical open lookup only when we don't already have a stored open
    if api_key and gameday and (open_s is None or open_t is None):
        try:
            gd = pd.to_datetime(str(gameday)[:10], errors="coerce")
            if pd.notna(gd):
                # NFL sides often open ~6–7 days out; try earliest first
                for days_back in (7, 6, 5, 8, 4, 9, 3, 2):
                    for hour in ("12:00:00", "17:00:00", "21:00:00"):
                        snap = gd - pd.Timedelta(days=days_back)
                        iso = snap.strftime(f"%Y-%m-%dT{hour}Z")
                        ...
                        if open_s is None and s is not None:
                            open_s = float(s)
                        if open_t is None and t is not None:
                            open_t = float(t)
    ...
    # First-seen live line becomes open ONLY if we still have no open
    if open_s is None and cur_s is not None:
        open_s = cur_s
    ...
    # Update last-known current; never overwrite a stored open with a newer current
    if cur_s is not None:
        entry["last_spread"] = cur_s
    if cur_t is not None:
        entry["last_total"] = cur_t
    if open_s is not None:
        entry["open_spread"] = open_s  # keep first real open (already set)
    if open_t is not None:
        entry["open_total"] = open_t
    # Protect: if entry already had open, don't replace with different current-driven value
    # (open_s/open_t already preferred stored values above)

    entry["source"] = source
    entry["first_seen"] = entry.get("first_seen") or datetime.now().strftime("%Y-%m-%d %H:%M")
    entry["updated"] = datetime.now().strftime("%Y-%m-%d %H:%M")
    opens[line_key] = entry
    _save_line_opens(opens)

    # Current display: live first, else last known (not open, unless that's all we have)
    if cur_s is None and entry.get("last_spread") is not None:
        try:
            cur_s = float(entry["last_spread"])
        except Exception:
    ...
    spread_move = round(float(cur_s) - float(open_s), 1) if (cur_s is not None and open_s is not None) else None
    total_move = round(float(cur_t) - float(open_t), 1) if (cur_t is not None and open_t is not None) else None
...
def bulk_opening_lines(api_key: str, game_keys: tuple) -> Dict[str, Dict]:
    """
    game_keys: tuple of (home, away, gameday) strings.
    Returns map key -> {open_spread, open_total, source} from historical API.
    Uses the earliest successful snapshot per matchup.
    """
    out: Dict[str, Dict] = {}
    if not api_key or not game_keys:
        return out
    # Group by gameday to limit API calls
    by_day: Dict[str, list] = {}
    for home, away, gd in game_keys:
        by_day.setdefault(str(gd)[:10], []).append((home, away))
    for gd, matchups in by_day.items():
        try:
            day = pd.to_datetime(gd, errors="coerce")
            if pd.isna(day):
                continue
            found = set()
            for days_back in (7, 6, 5, 8, 4, 3):
                iso = (day - pd.Timedelta(days=days_back)).strftime("%Y-%m-%dT17:00:00Z")
                hist = fetch_historical_nfl_odds(api_key, iso)
                if not hist:
                    continue
                for home, away in matchups:
                    ...
                    if s is None and t is None:
                        continue
                    out[key] = {
                        "open_spread": float(s) if s is not None else None,
                        "open_total": float(t) if t is not None else None,
                        "source": f"historical_{days_back}d",
                    }
                    found.add(key)
                if len(found) >= len(matchups):
                    break
        except Exception:
            continue
    return out
...
def load_nflverse_games_csv() -> pd.DataFrame:
    """Historical games with pre-game spread_line / total_line from nflverse."""
    urls = [
        "https://github.com/nflverse/nfldata/raw/master/data/games.csv",
...
def build_team_history(seasons: Optional[List[int]] = None) -> pd.DataFrame:
    ...
    Spreads/totals = nflverse pre-game lines (last available line before kickoff).
    """
    try:
        try:
            current = int(nfl.get_current_season())
        ...
        # Prefer nflverse games.csv for reliable spread_line / total_line
        s = load_nflverse_games_csv()
        if s is None or s.empty:
            s = load_schedules(seasons=seasons)
        if s is None or s.empty:
            return pd.DataFrame()
        s = s.copy()
        ...
        if "game_type" in s.columns:
            s = s[s["game_type"].astype(str).str.upper().str.contains("REG", na=False)]
        if s.empty:
            return pd.DataFrame()
