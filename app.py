...
# -----------------------------
# TEAM NAME MAPPING (Odds API full name → nflverse abbr)
# -----------------------------
TEAM_NAME_TO_ABBR = {
...
def get_team_epa(seasons=None):
    try:
        if seasons is None:
            ...
            # Prefer last 2 full-ish seasons + current if available
...
def fetch_nfl_odds(api_key: str):
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            return data, f"OK – {len(data)} events (requests remaining: {remaining})"
        else:
...
with tab1:
    ...
    if odds_data and not team_epa.empty:
        ...
        for game in odds_data:
                ...
                for market in book.get("markets", []):
                    if market["key"] == "spreads":
                        for o in market["outcomes"]:
                            if o["name"] == home_full:  # match on full name from API
            ...
            if signals or total_score > 2.0:  # slightly lower threshold so something shows
        ...
        if opportunities:
            ...
            st.markdown("""
            - **Rule score**: EPA / rest / underdog / large-spread / high-total  
            - **ML edge**: logistic regression on recent seasons (home ATS)  
            - **Monte Carlo edge**: simulated margins & totals → cover probs & EV at -110  
            - **Agreement bonus**: when ML and Monte Carlo agree  
            """)
        else:
            st.warning("No opportunities passed the score filter. Try lowering thresholds or check diagnostics above.")
            if skipped:
        ...
        if not api_key:
            ...
            st.error("Could not load team EPA data from nflreadpy. Check internet / package version.")
        else:
...
with tab3:
    ...
    st.write("Select a game to load available player props.")
    if not api_key:
        ...
        if st.button("Load Player Props for this game", type="primary"):
            ...
            if props_data is None:
                ...
                if rows:
                    ...
                    st.warning("No player props returned (plan limitation or no markets yet).")
...
# ========== TAB 5 placeholder ==========
with tab5:
    st.write("Deployment notes / Streamlit Cloud tips can go here.")
